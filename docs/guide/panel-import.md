...
4. 填**完整面板地址**（含端口）。同服务面板可写成 `http://host.docker.internal:8888/`；远程面板则填你浏览器能打开的地址
5. 填 API 密钥
...
::: tip
如果流盾 WAF 和宝塔为同一台服务器，则将Docker网关地址（一般为 `172.17.0.1`）加入白名单
:::
...
导入后仍可在站点编辑里改回源。源站如果只提供 HTTPS，导入后把回源协议改成 HTTPS。
...
1. 打开 **资产 → 证书管理**
2. 右上角点 **从其他面板导入**
3. 抽屉里切到宝塔或 1Panel；未设置时点「前往系统设置」
4. 勾选证书后导入

相同 PEM（按证书指纹）会跳过，不会重复入库。
...
核对地址（协议、端口、宝塔安全入口）、密钥、TLS 勾选。同机账号地址应是容器能访问的，例如 `http://host.docker.internal:8888/入口`（一键安装就是这样写的）。不要把面板地址填成容器内的 `127.0.0.1`——那是容器自己，不是宿主机上的宝塔 / 1Panel。
...
不会。再执行一次证书导入（相同指纹会跳过），或在证书管理里更新 PEM。
