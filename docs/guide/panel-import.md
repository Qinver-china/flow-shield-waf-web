...
4. 填**完整面板地址**（含端口）。宝塔只需 `http://host.docker.internal:8888` 这种「协议 + 主机 + 端口」，不必带安全入口；远程面板则填你浏览器能打开的地址（同样不用入口路径）
5. 填 API 密钥
...
核对地址（协议、端口）、密钥、TLS 勾选。宝塔地址不要带安全入口，同机账号应写成容器能访问的 `http://host.docker.internal:8888`（一键安装就是这样写的）。不要把面板地址填成容器内的 `127.0.0.1`——那是容器自己，不是宿主机上的宝塔 / 1Panel。
