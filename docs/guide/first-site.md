...
1. （要 HTTPS）可先上传证书，或从宝塔 / 1Panel 导入；也可以先用 HTTP 把站点接上、DNS 指到本机后，再在证书管理里 [申请免费证书](./certificates.md#acme)  
...
1. 打开 **证书管理**，上传或粘贴证书和私钥  
2. 本机已有宝塔 / 1Panel 证书时，按 [从其他面板导入](./panel-import.md) 导入  
3. 站点已保存、域名已解析到本机且 80 端口可达时，在证书管理切到 **申请免费证书**（Let's Encrypt / ZeroSSL），详见 [证书管理](./certificates.md#acme)

可选：打开到期提醒（需先配好邮件）；免费证书建议同时打开到期前自动续期。网站还在宝塔 / 1Panel 上时，可再打开续期后同步，见 [续期后同步到宝塔 / 1Panel](./certificates.md#panel-push)。稍后在站点里选这张证书。
