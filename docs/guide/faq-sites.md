...
核对地址（协议、端口；宝塔不必带安全入口）、API 密钥、是否关闭本机自签证书的 TLS 校验。容器访问宿主机宝塔时，API 白名单需要包含容器自身 IP（以及 `127.0.0.1`），而不是只加 `172.17.0.1`。一键安装会自动写入；手动排查可用 `docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' flowshield-waf-app`。1Panel 需先开启 API 接口并把 ApiKey 填进流盾。
